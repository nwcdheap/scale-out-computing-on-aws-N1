...
def windows_auto_stop_instance(instances_to_check):
    ...
    with db.app.app_context():
        ...
        for instance_id, instance_data in get_host_to_stop.items():
            logger.info("check instance_id:"+instance_id+"==========================")
                ...
                for instance_id in get_host_to_stop.keys():
                    ...
                    if ssm_failed is False:
                        try:
                            ...
                            if "DCVLastDisconnectTime" not in session_info.keys() or session_info["DCVLastDisconnectTime"] == "":
                                            ...
                                            if e.response['Error'].get('Code') == 'DryRunOperation':
                                                ...
                                                try:
                                                    ...
                                                    if check_session:
                                                        ...
                                                        response_ec2 = client_ec2.describe_tags(
                                                            DryRun=False,
                                                            Filters=[
                                                                {'Name': 'resource-id','Values': [instance_id]},
                                                                {'Name':'key','Values': ['soca:JobOwner']}
                                                            ]
                                                        )
                                                        logger.info("response_ec2:"+json.dumps(response_ec2))
                                                        log_data={"user":response_ec2["Tags"][0]["Value"],"instance_id":instance_id,"os":"windows","operation":"stop","operator":"system"}
                                                        mylog(log_data)
                                                    else:
                            ...
                            logger.error("windows_auto_stop_instance error:{}".format(e))
                    else:
                ...
                for instance_id in get_host_to_stop.keys():
                    ...
                    if ssm_failed is False:
                        ...
                        while ssm_list_command_loop < 6:
                            try:
                                ...
                                logger.error("check_command_status error:{}".format(e))
                                time.sleep(20)
                                ssm_list_command_loop += 1
                                continue
                            if check_command_status != "Success":
                    ...
                    if ssm_failed is False:
                        try:
                            ...
                            if "last-disconnection-time" not in session_info["DCV"].keys() or session_info["DCV"]["last-disconnection-time"] == "":
                                # handle case where user launched DCV but never accessed it
                                            ...
                                            if e.response['Error'].get('Code') == 'DryRunOperation':
                                                ...
                                                try:
                                                    ...
                                                    if check_session:
                                                        ...
                                                        log_data={"user":session_info["DCV"]["owner"],"instance_id":instance_id,"os":"Linux","operation":"stop","operator":"system"}
                                                        mylog(log_data)
                                                    else:
                            ...
                            logger.error("linux_auto_stop_instance error:{}".format(e))
                    else:
...
def schedule_auto_stop():
    ...
    for distribution in ["windows", "linux"]:
        ...
        for query in ["norun", "schedule"]:
            ...
            logger.info("**************")
            logger.info(f"distribution:{distribution},query:{query},instances_ids_to_check:{instances_ids_to_check}")
            logger.info("==========")
