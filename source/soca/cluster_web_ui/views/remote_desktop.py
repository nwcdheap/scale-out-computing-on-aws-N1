...
soca_event = dynamodb.Table(os.environ["SOCA_CONFIGURATION"])
...
def mylog(log_data):
    log_data["gmt_create"]=datetime.now(timezone.utc).astimezone(mytz).strftime("%Y-%m-%d %H:%M:%S")
    log_data["type"]="desktop"
    log_data["os"]="Linux"
    soca_event.put_item(Item=log_data)
...
then
    ...
    curl -o /etc/yum.repos.d/CentOS-Base.repo https://nwcd-samples.s3.cn-northwest-1.amazonaws.com.cn/software/CentOS-Base-china.repo
    yum clean all
    yum makecache
fi
...
then
    ...
    PIP=$(which pip3)
    $PIP install awscli==1.18.185 -i https://opentuna.cn/pypi/web/simple/
...
mkdir -p /data
# Mount EFS
echo "''' + soca_configuration['EFSDataDns'] + ''':/data /data nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport 0 0" >> /etc/fstab
echo "''' + soca_configuration['EFSAppsDns'] + ''':/apps /apps nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport 0 0" >> /etc/fstab
EFS_MOUNT=0
mount -a 
...
$AWS s3 cp s3://$SOCA_INSTALL_BUCKET/$SOCA_INSTALL_BUCKET_FOLDER/scripts/config.cfg /root/ --region=cn-northwest-1 --no-sign-request
/bin/bash /apps/soca/$SOCA_CONFIGURATION/cluster_node_bootstrap/ComputeNode.sh ''' + soca_configuration['SchedulerPrivateDnsName'] + ''' >> $SOCA_HOST_SYSTEM_LOG/ComputeNode.sh.log 2>&1'''
    ...
    db.session.commit()
    log_data={"user":str(session["user"]),"operation":"create","operator":"user"}
    mylog(log_data)
    return redirect("/remote_desktop")
...
def delete():
    ...
    if check_session:
        log_data={"user":str(session["user"]),"instance_id":check_session.session_instance_id,"operation":action,"operator":"user"}
        mylog(log_data)
        ...
        try:
            ...
            if e.response['Error'].get('Code') == 'DryRunOperation':
                try:
                    ...
                    log_data={"user":str(session["user"]),"instance_id":instance_id,"operation":"start","operator":"user"}
                    mylog(log_data)
                except Exception as err:
